<template>
  <div class="" style="background-color: #000;" :style="'height:' + windowHeight + 'px'">
    <el-tabs style="margin-top:0px;" v-model="activeName" class="black-style">
      <!-- label="美股"  -->
      <el-tab-pane  :name="['first'].includes(activeName) ? activeName : ''">
        <div class="table transaction-table">
          <div class="table-search">
            <el-row type="flex" justify="end">
              <el-col :span="24">
                <el-input size="mini" clearable v-model="form.stock" :placeholder="$t('tradingFloor.search')">
                  <el-button size="mini" @click="getList" slot="append" icon="el-icon-search"></el-button>
                </el-input>
              </el-col>
            </el-row>
          </div>
          <el-table v-loading="loading" :data="list.list" @row-click="toTransaction" :height="windowHeight"
            style="width: 100%">
            <el-table-column prop="name" :label="$t('common.stockName')">
              <template slot-scope="scope">
                <div class="tab-name">
                  <p style="display: flex; width: 100px;">
                    <a v-if="scope.row.isOption == 0" href="javascript:;" @click="tianjiaOptions(scope.row)"><i
                        class="red-xx iconfont icon-wujiaoxing"></i></a>
                    <a v-if="scope.row.isOption == 1" href="javascript:;" @click="quxiaoOptions(scope.row)"><i
                        class="red-xx iconfont icon-wujiaoxing1"></i></a>
        ...
        <!-- label="台股"  -->
      <el-tab-pane  :name="['four'].includes(activeName) ? activeName : ''">
        <div class="table transaction-table">
          <div class="table-search">
            <el-row type="flex" justify="end">
              <el-col :span="24">
      ...
      <el-tab-pane  v-if="$store.state.haslogin" name="second">
        <div v-show="$store.state.haslogin" class="table transaction-table">
          <div class="table-search">
            <el-row type="flex" justify="end">
              <el-col :span="24">
                <el-input size="mini" clearable v-model="form2.stockMy" :placeholder="$t('tradingFloor.search')">
                  <el-button size="mini" @click="getList" slot="append" icon="el-icon-search"></el-button>
                </el-input>
              </el-col>
            </el-row>
          ...
          <el-table v-loading="loading" :data="list.list" :height="windowHeight"
            style="width: 100%">
            <el-table-column prop="name" width="20px" v-if="$store.state.haslogin">
              <template slot-scope="scope">
                <a v-if="scope.row.isOption == 0" href="javascript:;" @click="tianjiaOptions(scope.row)"><i
                    class="red-xx iconfont icon-wujiaoxing red-xx"></i></a>
                <a v-if="scope.row.isOption == 1" href="javascript:;" @click="quxiaoOptions(scope.row)"><i
                    class="red-xx iconfont icon-wujiaoxing1 red-xx"></i></a>
            ...
            <el-table-column prop="name" :label="$t('common.stockName')">
              <template slot-scope="scope">
                <div class="tab-name" @click="toTransaction(scope.row)">
                  <p style="display: flex; width: 100px;">

                    <span style="text-overflow: ellipsis;overflow: hidden; white-space:nowrap;"> {{ scope.row.stockName
}}</span>
                  </p>
                  <span class="code">
...
export default {
  ...
  methods: {
    ...
    //   }
    // },
    async formatUsStockList(opt, resultData) {
      let data = {}
      if (resultData) {
        data = resultData
      } else {
    ...
    async formatTwStockList(opt, resultData) {
      ...
      if (resultData) {
        ...
        data = await api.getTwStockList(opt);
      }
      this.list = { ...data.data };
      this.total = this.list.total
    }
