    ...
    <div class="table">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <p class="bounceIn">
              <span :class="scope.row.profitAndLose<0?'green':'red'">{{scope.row.profitAndLose}} <i class="mini-size">{{scope.row.coinCode}}</i></span>
            </p>
            <p class="mini-font">
              ≈ {{(scope.row.profitAndLose * scope.row.sellRate).toFixed(2)}} USD
            </p>
          </template>
        </el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <p class="bounceIn">
              <span :class="scope.row.allProfitAndLose<0?'green':'red'">{{scope.row.allProfitAndLose}} <i
                class="mini-size">{{scope.row.coinCode}}</i></span>
            </p>
            <p class="mini-font">
              ≈ {{(scope.row.allProfitAndLose * scope.row.sellRate).toFixed(2)}} USD
            </p>
          </template>
        </el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <span class="mini-size">{{scope.row.coinCode}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="sellOrderPrice"
...
export default {
  ...
  methods: {
    ...
    async getList () {
      ...
      this.loading = true
      let data = await api.getUserFuturesPositionList(opts)
      this.loading = false
      if (data.status === 0) {
        this.list = data.data
      } else {
