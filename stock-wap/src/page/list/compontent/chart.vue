...
export default {
  ...
  methods: {
    ...
    valuesVolumes (data) {
      // 處理成交量 現在返回的成交量是每一次分鐘返回的總量
      // 用下一個數據減去上一個數據得到這分鐘的成交量
      let newData = []
    ...
    initEchartMap (data) {
      ...
      let option = {
        ...
        animation: true,
        // legend: { // 圖例組件
        //     bottom: 10, // 圖例組件離容器下側的距離。
        //     left: 'center', // 圖例組件離容器左側的距離。
        //     data: data.stockName
        // },
        tooltip: { // 提示框組件 坐標軸指示器是指示坐標軸當前刻度的工具
          trigger: 'axis', // 坐標軸觸發，主要在柱狀圖，摺線圖等會使用類目軸的圖
          axisPointer: {
            type: 'cross'
          },
          ...
          formatter: function (param) {
            ...
            // data.rates[] Quote change度 amounts 成交額
            if (param && param2) {
              return [
                ...
                'Quote change: ' + '<span class="' + classFont + '" style="font-size:16px;">' + data.rates[param.dataIndex] + '%</span>' + '<br/>',
        ...
        toolbox: { // 工具欄
          feature: { // 各工具配置項。//除了各個內置的工具按鈕外，還可以自定義工具按鈕。
            dataZoom: {// 數據區域縮放
              yAxisIndex: false
            },
            brush: { // 選框組件的控制按鈕
        ...
        yAxis: [
          {
            ...
            splitLine: {
              lineStyle: {
                // 使用深淺的間隔色
                color: this_.classStyle === 'red' ? '#eee' : '#333' // 分割線
              }
            }
          },
          {
