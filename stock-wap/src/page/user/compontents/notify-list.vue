        ...
        <li class="list-body" v-for="(item) in list" :key="item.id">
          <div class="order-info-box">
            <div class="order-title">
              ...
              <span class="payNumber">time:<span :style="{color:$state.theme =='red'?'#BB1815':''}">{{(item.addTime)}}</span></span>
              <span
                :class="item.status == 2?'green pull-right':item.status != 2?'red pull-right':'red pull-right'">
                        ...
                        <!-- 1 => 成功 2 失敗 3取消 4 等待 -->
                        {{item.status == 2?'Have read':'unread'}}
            ...
            <div class="order-info">
              ...
              <div class="info-mix ">
                <span class="info-item">order number:<b>{{item.content}}</b></span>
              </div>
            </div>
