...
//        }
        ...
        if (agentUser == null) {
            return ServerResponse.createByErrorMsg("用戶密碼不正確");
        }

        if (agentUser.getIsLock().intValue() == 1) {
            return ServerResponse.createByErrorMsg("登陆失敗，您的帳号已被锁定！");
        }
    ...
    public ServerResponse addAgentUser(String agentName, String agentPwd, String agentRealName, String agentPhone, Integer parentId, String poundageScale,  String deferredFeesScale, String receiveDividendsScale, HttpServletRequest request) {
        ...
        if (dbuser != null) {
            return ServerResponse.createByErrorMsg("添加失敗，代理名已存在");
        }

        AgentUser dbuser2 = this.agentUserMapper.findByPhone(agentPhone);
        if (dbuser2 != null) {
            return ServerResponse.createByErrorMsg("添加失敗，手機號已存在");
        }
    ...
    public ServerResponse update(AgentUser agentUser) {
        ...
        dbAgent.setPoundageScale(agentUser.getPoundageScale());
        dbAgent.setDeferredFeesScale(agentUser.getDeferredFeesScale());
        dbAgent.setReceiveDividendsScale(agentUser.getReceiveDividendsScale());
        ...
        if (StringUtils.isNotBlank(agentUser.getAgentPhone())) {
            ...
            if (phoneAgent == null || phoneAgent.getId().equals(agentUser.getId())) {
                dbAgent.setAgentPhone(agentUser.getAgentPhone());
            } else {
                return ServerResponse.createByErrorMsg("手機號已存在，請更换手机");
            }
        }
    ...
    private AgentInfoVO assembleAgentInfoVO(AgentUser agentUser, String host) {
        ...
        agentInfoVO.setPoundageScale(agentUser.getPoundageScale());
        agentInfoVO.setDeferredFeesScale(agentUser.getDeferredFeesScale());
        agentInfoVO.setReceiveDividendsScale(agentUser.getReceiveDividendsScale());
        String pcUrl = host + PropertiesUtil.getProperty("site.suffix.url") + agentUser.getAgentCode();
        agentInfoVO.setPcUrl(pcUrl);
        String _host = host.substring(7); //去掉协议http://的域名
        String mUrl =PropertiesUtil.getProperty("site.prefix.url") +
                _host + PropertiesUtil.getProperty("site.suffix.url") + agentUser.getAgentCode();
    ...
    public ServerResponse updateAgentAmt(Integer agentId, Integer amt, Integer direction) {
        ...
        agentAgencyFee.setRemarks("【提現支出】提現金额："+amt);
        ...
        target.append("代理id : ").append(agentUser.getId())
                .append("修改前 总資金 = ").append(totalMoney)
                .append("修改后 总資金 = ").append(back_amt);
