...
public class FundsApplyServiceImpl implements IFundsApplyService {
    ...
    public ServerResponse insert(FundsApply model, HttpServletRequest request) throws Exception {
        int ret = 0;
        if (model == null) {
            return ServerResponse.createBySuccessMsg("操作異常，請稍後重試！");
        }
        User user = this.iUserService.getCurrentRefreshUser(request);
        if(user == null){
            return ServerResponse.createBySuccessMsg("請登入後操作");
        }
        ...
        log.info("用戶可用金额 = {}  实际購買金额 =  {} 比较结果 = {} ", user_enable_amt, pay_amount, compareUserAmtInt);
        if (compareUserAmtInt == -1) {
            return ServerResponse.createByErrorMsg("申請失敗，可用金额小於" + pay_amount + "元");
        }
        ...
        if(ret>0){
            ...
            if (updateUserCount > 0) {
                ...
                ucd.setDeSummary("申請Daily allocation:" + model.getOrderNumber() + "，凍結金額：" + model.getPayAmount().multiply(new BigDecimal("-1")) );
                ucd.setAddTime(new Date());
                ucd.setIsRead(Integer.valueOf(0));
                int insertSxfCount = this.userCashDetailMapper.insert(ucd);
                if (insertSxfCount > 0) {
                    log.info("【Daily allocation】申請成功");
                }

            } else {
                log.error("【Daily allocation】修改用戶金额出错");
                throw new Exception("【Daily allocation】修改用戶金额出错");
            }
        ...
        /* 137 */     if (model != null && model.getId().intValue() > 0) {
            ...
            /* 161 */       if (ret > 0) {
                ...
                /* 164 */         if (model.getStatus().intValue() == 1) {
                    ...
                    /* 170 */           log.info("【配資審核通過】用戶平总資金  = {} , 可用資金 = {} , 总操盘資金 = {}", new Object[] { reckon_all, user_enable_amt, tradingAmount });
                    ...
                    /* 174 */           if (updateUserCount > 0) {
                        ...
                        /* 191 */             ucd.setDeSummary("申請Daily allocation:" + fundsApply.getOrderNumber() + "，配資審核通過，解凍Margin到配資帳戶：" + fundsApply.getPayAmount());
                        ...
                        /* 195 */             if (insertSxfCount > 0) {
                            ...
                            /*     */             }
                        /*     */           } else {
                        /* 199 */             log.error("【配資審核通過】修改用戶金额出错");
                        /* 200 */             throw new Exception("【配資審核通過】修改用戶金额出错");
                        /*     */           }
                    /*     */
                    /*     */         } else {
                    /*     */
                    ...
                    /* 208 */           if (updateUserCount > 0) {
                        /* 209 */             log.info("【配資審核Did not pass】修改用戶金额成功");
                        ...
                        /* 216 */             ucd.setDeType("配資審核Did not pass");
                        /* 217 */             ucd.setDeAmt(fundsApply.getPayAmount());
                        /* 218 */             ucd.setDeSummary("申請Daily allocation:" + fundsApply.getOrderNumber() + "，配資審核Did not pass，解凍Margin到余額：" + fundsApply.getPayAmount() + "，原因：" + model.getAuditOpinion());
                        ...
                        /* 222 */             if (insertSxfCount > 0) {
                            /* 223 */               log.info("【Daily allocation】申請成功");
                            /*     */             }
                        /*     */           } else {
                        /* 226 */             log.error("【Daily allocation】修改用戶金额出错");
                        /* 227 */             throw new Exception("【Daily allocation】修改用戶金额出错");
                        /*     */           }
                    /*     */         }
                /*     */
                /* 231 */         log.info("配資申請-審核 = {}  实际購買金额 =  {} 比较结果 = {} ", Integer.valueOf(0));
                /*     */       }
            /*     */     }
        /* 234 */     if (ret > 0) {
